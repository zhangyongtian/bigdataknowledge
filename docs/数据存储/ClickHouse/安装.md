...
ClickHouse**通过向量化执行引擎来加速查询，向量化执行可以简单的看作一项消除程序中循环的优化**，为了实现向量化需要利用 CPU 的 SIMD (Single Instruction Multiple Data)指令，**通过单条指令可以实现操作多条数据**。在现代计算机中是通过数据并行来提高性能，其原理就是在 CPU 寄存器层面实现数据的并行操作。ClickHouse 目前通过 SSE 4.2 指令集实现向量化执行的。
