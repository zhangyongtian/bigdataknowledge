...
| 主机          | 规划设置主机名 | 角色                                            |
|--------------|-----------------  |-------------------------------------------------|
| 192.168.61.200  | master1           | NameNode、ResourceManager |
| 192.168.61.201 | master2           | NameNode、ResourceManager       |
| 192.168.61.202 | hadoop1             | DataNode、NodeManager                           |
| 192.168.61.203 | hadoop2             | DataNode、NodeManager                           |
| 192.168.61.204 | hadoop3             | DataNode、NodeManager                           |

hadoop1，hadoop2，hadoop3 都有DataNode、NodeManager，所以修改如下。
...
```
mkdir /home/bigdata/module/hadoop-3.2.3/data
mkdir /home/bigdata/module/hadoop-3.2.3/data/name
mkdir /home/bigdata/module/hadoop-3.2.3/data/jn
sudo chown bigdata:bigdata -R /home/bigdata/module/hadoop-3.2.3/data
sudo chmod 744 /home/bigdata/module/hadoop-3.2.3/data
```
...
<value>qjournal://hadoop1:8485;hadoop2:8485;hadoop3:8485/bigdatacluster</value>
  </property>
...
### 容量调度器
capacity-scheduler.xml （默认就是容量调度器）
...
<property>
    ...
    <value>/home/bigdata/module/hadoop-3.2.3/etc/hadoop/fair-scheduler.xml</value>
...
添加公平调度器配置文件
```shell
sudo vim /home/bigdata/module/hadoop-3.2.3/etc/hadoop/fair-scheduler.xml
```
```xml
<?xml version="1.0" encoding="UTF-8"?>
<allocations>
...
<queue name="root" >
    <queue name="default">
        ...
        <!-- 队列最大资源 -->
        <maxResources>51200mb,28vcores</maxResources>
...
chmod 744 /home/bigdata/shell/hadoop_server.sh
...
"start")
        echo " =================== 启动 hadoop集群 ==================="
		echo " hadoop1 的journalnode启动"
        ssh hadoop1 "hdfs --daemon start journalnode"
        echo " hadoop2 的journalnode启动"
        ssh hadoop2 "hdfs --daemon start journalnode"
		echo " hadoop3 的journalnode启动"
        ssh hadoop3 "hdfs --daemon start journalnode"
...
"stop")
		...
		echo " hadoop1 的journalnode关闭"
        ssh hadoop1 "hdfs --daemon stop journalnode"
        echo " hadoop2 的journalnode关闭"
        ssh hadoop2 "hdfs --daemon stop journalnode"
		echo " hadoop3 的journalnode关闭"
        ssh hadoop3 "hdfs --daemon stop journalnode"
;;
*)
...
### 访问地址
```
http://master1:8088/cluster/scheduler
http://master1:9870/dfshealth.html#tab-overview
```
