...
### 设置副本
```
ALTER TABLE person MODIFY PARTITION (*) SET("replication_num"="3");
<!-- #执行完修改以后过一会才有用 -->
ADMIN SHOW REPLICA STATUS FROM person;
```
...
+-------------------+--------+
| cluster_load_stat | 1      |
| working_slots     | 5      | (如果不是0，那么就是在调度中)
| sched_stat        | 1      |
| priority_repair   | 0      |
| pending_tablets   | 1931   |
| running_tablets   | 0      |
...
```
SHOW VARIABLES LIKE '%disable_balance%';
```
