...
BigdataKnowledge支持以配置的形式集成第三方组件。当前以集成Kafka组件为例，详细介绍服务集成协议。
...
```
{
  ...
  "decompressPackageName": "kafka-2.4.1",    #服务安装包解压后名称，默认安装在/opt/BigdataKnowledge目录下
  "roles": [                                 #服务角色列表
    {
      ...
      "logFile": "logs/server.log",      #KafkaBroker角色日志路径，/opt/BigdataKnowledge/kafka-2.4.1/logs/server.log
      "jmxPort": 9991,
      "startRunner": {
        "timeout": "60",
...
参考该协议，可以很方便的集成第三方组件，将第三方组件的配置和服务监控，启停管理托管到BigdataKnowledge上。
